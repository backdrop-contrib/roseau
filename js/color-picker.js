/**
 * @file
 * Provides UI/UX progressive enhancements on Roseau's theme settings by
...
((Drupal, settings, once) => {
  const colorSchemeOptions = settings.roseau.colorSchemes;
  ...
  function synchronizeInputs(changedInput, inputToSync) {
    ...
    changedInput.setAttribute('data-roseau-custom-color', changedInput.value);
    inputToSync.setAttribute('data-roseau-custom-color', changedInput.value);
  ...
  function setColorScheme({ target }) {
    ...
    if (selectedColorScheme) {
      ...
      document
        .querySelectorAll(`input[data-roseau-custom-color]`)
        .forEach((input) => {
          input.value = input.getAttribute('data-roseau-custom-color');
        });
    }
  }
  ...
  /**
   * Initializes Roseau theme-settings color picker.
  ...
  function initColorPicker(textInput) {
    ...
    colorInput.setAttribute(
      'data-roseau-custom-color',
      textInput.getAttribute('data-roseau-custom-color'),
    );
  ...
  /**
   * Roseau Color Picker behavior.
   *
   * @type {Drupal~behavior}
   * @prop {Drupal~behaviorAttach} attach
   *   Initializes color picker fields.
   */
  Drupal.behaviors.roseauColorPicker = {
    attach: () => {
      ...
      const colorTextInputs = once(
        'roseau-color-picker',
        '[data-drupal-selector="roseau-color-picker"] input[type="text"]',
      );
